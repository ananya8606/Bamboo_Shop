...
export const createProduct = createAsyncThunk('products/createProduct', async (productData,{ getState,rejectWithValue }) => {
  ...
  const { data } = await api.post('api/products/productCreate', productData,config);
  console.log(data)
  return data;
} catch (error) {
  return rejectWithValue(
...
export const productsSearch = createAsyncThunk(
  ...
  async (productName, { rejectWithValue }) => {
    try {
      const { data } = await api.get(`/api/products/search/${productName}`);
      return data;
    } catch (error) {
      return rejectWithValue(
        error.response && error.response.data.message
  ...
  async (id, { getState, rejectWithValue }) => {
      try {
        const userInformation = getState().user.userLogin.userInformation;
          ...
          await api.delete(`/api/products/product/${id}`, config);
...
export const uploadImage = createAsyncThunk(
  ...
  async (formData, { getState,rejectWithValue }) => {
    try {
      ...
      const { data } = await api.post('/api/products/uploadImage', formData,config);
      return data;
    } catch (error) {
      return rejectWithValue(
        error.response && error.response.data.message
